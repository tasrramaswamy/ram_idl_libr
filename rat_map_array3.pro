...
pro rat_map_array3,arr, lat, lon, min_val, max_val, n_colors, pngfile,$
   ...
   _reverse = _reverse ,stop1=stop1, toz_colors=toz_colors
    ...
    !p.charsize=1.4
    ;load_colors
	if (KEYWORD_SET(toz_colors)) then begin
		levels = rat_v8_ozone_colors()
		min_val = levels[0]
		max_val = levels[-1]
		n_colors = n_elements(levels)-1
	endif else begin
