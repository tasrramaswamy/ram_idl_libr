pro rat_plot_13ch,X1,Y1,dev=dev,ps_name=ps_name,xtit=xtit,ytit=ytit,tit=tit,leg=leg,$
	colors=colors,ct=ct,xrange=xrange,yrange=yrange,lines=lines1,$
	xmin=xmin,ymin=ymin,xtick=xtick,ytick=ytick, xlabel=xlabel
;
;
;
;
...
if ~KEYWORD_SET(colors) then COLORS = [ fltarr(4)+1,fltarr(4)+2, fltarr(4)+3, 4 ]
if ~KEYWORD_SET(lines) then lines1 = indgen(13) mod 4

dev=STRUPCASE(dev)

if(dev ne'PS') then rat_window else begin
...
PLOT,X,Y(*,0),/nodata,/noerase,xtitle=xtit,ytitle=ytit,title=tit,$
	...
	xminor=xmin,yminor=ymin,xtickn=xlabel
