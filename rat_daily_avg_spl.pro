pro rat_daily_avg_spl, daynum, a,uniq_days,b,$
	day_dim_no=day_dim_no,fill_value=fill_value,$
	 max_dev_med = max_dev_med ,max_dev_avg=max_dev_avg,$
	 min_dev_med = min_dev_med,min_dev_avg = min_dev_avg,stop1=stop1
...
count=fltarr(n_uniq)
	...
	case n_other of 
	0: 	begin
		out[i]= rat_avg(inp[idx], max_dev_med = max_dev_med ,max_dev_avg=max_dev_avg,$
	min_dev_med = min_dev_med,min_dev_avg = min_dev_avg,fill_val=fill_value)
		end
	else:	begin
		for j=0,n_other -1 do begin
			out[i,j] = rat_avg(inp[idx,j], max_dev_med = max_dev_med ,max_dev_avg=max_dev_avg,$
	min_dev_med = min_dev_med,min_dev_avg = min_dev_avg,fill_val=fill_value)
		endfor
		end
	endcase
